defmodule CasinoWeb.GameLive.Index do
  ...
  def handle_event("deal_cards", _url, socket) do
    ...
    socket =
      ...
      |> update(:dealer_score, fn _score ->
        dealer_score(updated_game.dealer, socket.assigns.show_dealer)
      end)
  ...
  def handle_event("show_dealer", _url, socket) do
    ...
    {:noreply,
     update(
       updated_socket,
       :dealer_score,
       fn _score -> dealer_score(game.dealer, updated_socket.assigns.show_dealer) end
     )}
  end
